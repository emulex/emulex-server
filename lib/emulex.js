...
function bootstrap(conf, cback) {
    ...
    db.open(conf.dbpath, function (err, tdb) {
        ...
        edb.search("ex_server", { type: SS_ED2K }, function (err, back) {
            console.log("emulex: having " + back.length + " ed2k server");
            if (back.length < 1) {
                emulex.connected = false;
                return;
            }
            back.forEach(function (srv, idx) {
                emulex.ed2k_server_connect(srv.name, srv.addr, srv.port, idx === 0);
            });
